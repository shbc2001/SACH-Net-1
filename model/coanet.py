...
from .connect import build_connect
from .save_feature_map import save_map
def custom_group_norm(num_channels):
    if(num_channels==9):
        num_groups=num_channels//3
    else:
        num_groups = num_channels//4
    return nn.GroupNorm(num_groups, num_channels)

class CoANet(nn.Module):
    def __init__(self, backbone='resnet', output_stride=16, num_classes=2, num_neighbor=5,
